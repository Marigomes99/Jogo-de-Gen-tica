...
class Jogo {
    ...
    public void iniciar() {
        for (Pergunta pergunta : perguntas) {
            String resposta = exibirPergunta(pergunta);
    ...
    private String exibirPergunta(Pergunta pergunta) {
        StringBuilder sb = new StringBuilder();
        sb.append(pergunta.getTexto()).append("\n");
        for (int i = 0; i < pergunta.getRespostas().length; i++) {
            sb.append(i + 1).append(". ").append(pergunta.getRespostas()[i]).append("\n");
        }
        String resposta = JOptionPane.showInputDialog(sb.toString());
        return pergunta.getRespostas()[Integer.parseInt(resposta) - 1];
    }

    private void exibirResultado() {
        JOptionPane.showMessageDialog(null, "Você acertou " + acertos + " de " + perguntas.size() + " perguntas.");
    }
}
